...
class Settings {
  ...
  static Future<void> load() async => _prefs = await SharedPreferences.getInstance();
  ...
  static String get preferredSource => _prefs.getString('preferred_source') ?? '';
  static set preferredSource(String v) => _prefs.setString('preferred_source', v);
  ...
  static SkipMode get skipMode => SkipMode.values.asNameMap()[_prefs.getString('skip_mode')] ?? SkipMode.button;
  ...
  static String get subtitleLanguage => _prefs.getString('subtitle_language') ?? 'English';
  static set subtitleLanguage(String v) => _prefs.setString('subtitle_language', v);
...
const subtitleLanguages = ['Off', 'English', 'Spanish', 'Portuguese', 'French', 'German', 'Italian', 'Arabic', 'Russian'];
...
class _SettingsScreenState extends State<SettingsScreen> {
  Future<void> _choose<T>(String title, Map<T, String> options, T current, void Function(T) apply) async {
            ...
            children: [
              ...
              for (final MapEntry(:key, :value) in options.entries)
                ListTile(
                  ...
                  title: Text(value),
                  trailing: key == current ? Icon(Icons.check_rounded, color: primary) : null,
  ...
  Future<void> _signIn() async {
    ...
    try {
      ...
      if (mounted && me != null) showSuccess(context, 'Signed in as ${me['name']}');
    } catch (e) {
        ...
        children: [
          _Group('AniList', [
            FutureBuilder(
              ...
              builder: (context, snap) {
                ...
                return ListTile(
                  ...
                  title: Text(signedIn ? (snap.data?['name'] ?? 'AniList') : 'Not signed in'),
                  subtitle: Text(signedIn ? 'Your progress syncs to AniList' : 'Sign in to track what you watch'),
                  trailing: signedIn
            ...
            _Choice(
              ...
              onTap: () => _choose('Count as watched at', {for (final p in const [70, 80, 85, 90, 95]) p: '$p%'},
                  Settings.watchedPercent, (v) => Settings.watchedPercent = v),
            ),
          ]),
          _Group('Playback', [
            _Choice(
              ...
              onTap: () => _choose('Preferred audio', const {false: 'Sub (Japanese audio)', true: 'Dub (English audio)'},
                  Settings.preferDub, (v) => Settings.preferDub = v),
            ),
            FutureBuilder(
              future: sites,
              builder: (context, snap) => _Choice(
                title: 'Preferred source',
                value: Settings.preferredSource.isEmpty ? 'Highest ranked' : Settings.preferredSource,
                onTap: () => _choose(
                  'Preferred source',
                  {'': 'Highest ranked', for (final s in snap.data ?? const <Source>[]) s.name: s.name},
            ...
            _Choice(
              ...
              value: const {SkipMode.button: 'Show button', SkipMode.auto: 'Automatically', SkipMode.off: 'Off'}[
                  Settings.skipMode]!,
              onTap: () => _choose(
                'Skip intro & outro',
                const {SkipMode.button: 'Show a skip button', SkipMode.auto: 'Skip automatically', SkipMode.off: 'Off'},
                Settings.skipMode,
            ...
            _Choice(
              ...
              onTap: () => _choose('Double-tap to seek', {for (final s in const [5, 10, 15, 30]) s: '$s seconds'},
                  Settings.seekSeconds, (v) => Settings.seekSeconds = v),
            ),
            _Choice(
              title: 'Default speed',
              value: '${Settings.speed}×',
              onTap: () => _choose('Default speed', {for (final s in const [.75, 1.0, 1.25, 1.5, 2.0]) s: '$s×'},
          ...
          _Group('Subtitles', [
            _Choice(
              ...
              onTap: () => _choose('Subtitle language', {for (final l in subtitleLanguages) l: l},
                  Settings.subtitleLanguage, (v) => Settings.subtitleLanguage = v),
            ),
            _Choice(
              title: 'Size',
              value: {18.0: 'Small', 22.0: 'Medium', 28.0: 'Large'}[Settings.subtitleSize] ?? 'Medium',
              onTap: () => _choose('Subtitle size', {18.0: 'Small', 22.0: 'Medium', 28.0: 'Large'},
                  Settings.subtitleSize, (v) => Settings.subtitleSize = v),
            ),
          ]),
          _Group('Storage', [
            ListTile(
              ...
              subtitle: const Text('Removes continue-watching positions on this device'),
              onTap: () async {
                if (!await _confirm('Clear watch history?', 'Saved positions will be removed.', 'Clear')) return;
                await WatchHistory.clear();
                if (context.mounted) showSuccess(context, 'Watch history cleared');
              },
            ),
            ListTile(
            ...
            ListTile(
              ...
              subtitle: const Text('Use this if a site keeps failing after verifying'),
              onTap: () async {
                await CookieManager.instance().deleteAllCookies();
                if (context.mounted) showSuccess(context, 'Site verifications cleared');
              },
            ),
          ]),
          const Padding(
            ...
            child: Text(
              ...
              style: TextStyle(fontSize: 12, color: Colors.white38, height: 1.6),
            ),
          ),
        ],
      ),
...
class _Choice extends StatelessWidget {
  const _Choice({required this.title, required this.value, required this.onTap, this.subtitle});
